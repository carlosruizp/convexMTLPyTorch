...
def get_estim(estim_name, task=None, progress_bar=False):
    ...
    if strategy == 'hs':
        estim = HardSharingMTLPytorchClassifier(enable_progress_bar=progress_bar)
    elif strategy == 'cvx':
        estim = ConvexMTLPytorchClassifier(lambda_trainable=True, specific_lambda=False, enable_progress_bar=progress_bar)
    elif strategy == 'gl':
        estim = GraphLaplacianMTLPytorchClassifier(adj_trainable=True, enable_progress_bar=progress_bar)
    elif strategy == 'common':
        estim = ConvexMTLPytorchClassifier(lambda_trainable=False, specific_lambda=False, lamb=1, enable_progress_bar=progress_bar)
    elif strategy == 'ind':
        estim = ConvexMTLPytorchClassifier(lambda_trainable=False, specific_lambda=False, lamb=0, enable_progress_bar=progress_bar)
